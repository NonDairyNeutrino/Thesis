...
\title{\Large \vspace{-0.625in} \emph{Production of Massless Spin-0 Phonons in Analog (3+1) Inflationary Cosmologies Using High-Performance Finite-Differences} \vspace{-0.15in}}
...
\pagebreak
    ...
    \subsection{Computational Implementation}
        ...
        \subsubsection{Conformal Time Integration}
            ...
            Because the expansion of the universe is non-zero for a finite amount of (lab) time, the conformal time can be precomputed for all discretized time steps.  The conformal time $\eta$ is defined by the first order differential equation $\frac{d\eta}{dt} = \sqrt{a(t)}$.  As such, the conformal time can be simply integrated as 
            ...
            =======INCLUDE DISCUSSION ON ERROR=======

\pagebreak

        \subsubsection{Finite Difference Coefficients}

            Find finite difference coefficients on non-uniform grids (because conformal time is non-uniform from integration) using algorithms from literature\cite{FiniteDifferences}.

        \subsubsection{Discretized Operator}

            Because the field equation (eq. \ref{eq:fieldEquationConformalFourier}) is linear, it can be rewritten in ``operator form'' as

            \begin{equation} \label{eq:operator}
                \left[\partial_\eta^2 - \frac{1}{2} \frac{\dot{a}(\eta)}{a(\eta)} \partial_\eta - c_0^2 k^2 \right] \widetilde{\theta}_\mathbf{k} = 0.
            \end{equation}

            Representating the field equation in operator form naturally lends itself to being discretized as

            \begin{equation} \label{eq:operatordiscretized}
                \begin{bmatrix}
                ...
                \end{bmatrix} \begin{bmatrix}
                    \widetilde{\theta}_\mathbf{k}(\eta_0) \\
                    \widetilde{\theta}_\mathbf{k}(\eta_1) \\
                    \vdots \\
                    \widetilde{\theta}_\mathbf{k}(\eta_N) \\
                \end{bmatrix} = \begin{bmatrix}
                    0 \\
                    0 \\
                    \vdots \\
            ...
            Can use GPU-based sparse matrix via \verb|cuSPARSE|
