...
\title{\Large \vspace{-0.625in} \emph{Production of Massless, Spin-0 Quasiparticles with Linear Dispersion in Analog (3+1) Inflationary Cosmologies Using Finite-Differences and High-Performance Computing} \vspace{-0.15in}}
...
\pagebreak
        ...
        \subsubsection{The Field Equation}
            ...
            \begin{equation} \label{eq:fieldEquation2D}
                \text{2D: } \partial_t^2\Delta\theta - \frac{3}{2} \frac{\dot{b}(t)}{b(t)} \partial_t \Delta\theta - c_0^2 b(t) \nabla^2\Delta\theta = 0
            \end{equation}
            \begin{equation} \label{eq:fieldEquation3D}
                \text{3D: } \partial_t^2\Delta\theta - \phantom{\frac{3}{2}} \frac{\dot{b}(t)}{b(t)} \partial_t \Delta\theta - c_0^2 b(t) \nabla^2\Delta\theta = 0
            \end{equation}
            \end{subequations}
            ...
            \begin{equation} \label{eq:fieldEquationConformal}
                \partial_\eta^2 \Delta\theta - \frac{1}{2} \frac{\dot{b}(\eta)}{b(\eta)} \partial_\eta \Delta\theta - c_0^2 \nabla^2 \Delta\theta = 0.
            \end{equation}
        ...
        \subsubsection{Acoustic \& Bogoliubov Modes}
            ...
            \begin{equation}
                \Delta\theta(\mathbf{x}, t) = \frac{1}{\sqrt{V}} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{x}} \phi_\mathbf{k}(t) \hat{b}_\mathbf{k}(t) + e^{-i \mathbf{k} \cdot \mathbf{x}} \phi_\mathbf{k}^*(t) \hat{b}_\mathbf{k}^\dagger(t)
            \end{equation}
            \begin{equation}
                \Delta n(\mathbf{x}, t) = \frac{1}{\sqrt{V}} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{x}} \rho_\mathbf{k}(t) \hat{b}_\mathbf{k}(t) + e^{-i \mathbf{k} \cdot \mathbf{x}} \rho_\mathbf{k}^*(t) \hat{b}_\mathbf{k}^\dagger(t)
            \end{equation}
            \end{subequations}

            where $\mathbf{k}$ is the wave vector of the phase and density perturbations, 
...
\newpage
            ...
            \begin{equation} \label{eq:fieldEquationConformalFourier}
                \boxed{\partial_\eta^2 \phi_\mathbf{k} - \frac{1}{2} \frac{\dot{b}(\eta)}{b(\eta)} \partial_\eta \phi_\mathbf{k} - c_0^2 k^2 \phi_\mathbf{k} = 0.}
            \end{equation}
            ...
            \begin{equation} \label{eq:fieldEquationdeSitter}
                \partial_\eta^2 \phi_\mathbf{k} - \frac{1}{\eta} \partial_\eta \phi_\mathbf{k} + c_0^2 k^2 \phi_\mathbf{k} = 0,
            \end{equation}

            where the scaling function $b$ has been defined such that $b(t) = e^{-t / t_s}$ so that the analog universe is undergoing a de Sitter type expansion on a time scale $t_s$.  The boundary conditions for equation (\ref{eq:fieldEquationdeSitter}) come from the fact that $\phi_\mathbf{k}$ needs to be continuous at the moment when expansion begins.
        ...
        \subsubsection{Particle Production} \label{sec:particleProduction}
            ...
            \begin{equation} \label{eq:densityFromPhase}
                \rho_\mathbf{k} = - \frac{\hbar}{U} \partial_t \phi_\mathbf{k}.
            \end{equation}
            ...
            \begin{enumerate}
                \item Solve the field equation (\ref{eq:fieldEquationdeSitter}) for $\phi_\mathbf{k}(t)$
                \item Differentiate $\phi_\mathbf{k}(t)$ to get $\rho_\mathbf{k}(t)$ as in equation (\ref{eq:densityFromPhase})
                \item Combine $\phi_\mathbf{k}(t)$ and $\rho_\mathbf{k}(t)$ as in equations (\ref{eq:mixedFourierAmplitudes})
