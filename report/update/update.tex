...
\vfill
            ...
            We can expand the phase $\Delta\theta(\mathbf{x}, t)$ and density $\Delta n(\mathbf{x}, t)$ perturbation fields in a Fourier plane-wave amplitude representation as 

            \begin{subequations} \label{eq:FourierExpansion}
            \begin{equation}
                \Delta\theta(\mathbf{x}, t) = \frac{1}{\sqrt{V}} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{x}} \widetilde{\theta}_\mathbf{k}(t) \hat{b}_\mathbf{k}(t) + e^{-i \mathbf{k} \cdot \mathbf{x}} \widetilde{\theta}_\mathbf{k}^*(t) \hat{b}_\mathbf{k}^\dagger(t)
            \end{equation}
            \begin{equation}
                \Delta n(\mathbf{x}, t) = \frac{1}{\sqrt{V}} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{x}} \widetilde{n}_\mathbf{k}(t) \hat{b}_\mathbf{k}(t) + e^{-i \mathbf{k} \cdot \mathbf{x}} \widetilde{n}_\mathbf{k}^*(t) \hat{b}_\mathbf{k}^\dagger(t)
            \end{equation}
            \end{subequations}

            where $\mathbf{k}$ is the wave vector of the phase and density perturbations (does $\mathbf{k}$ physically represent something physically?), $\widetilde{\theta}_\mathbf{k}$ is the discrete Fourier Transform of the phase perturbation $\Delta \theta$, 

\newpage
            
            Equation (\ref{eq:fieldEquationConformal}) then becomes
