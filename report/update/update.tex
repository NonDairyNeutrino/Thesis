...
\usepackage[ruled,lined,linesnumbered,noend,nosemicolon]{algorithm2e}
...
\title{\Large \vspace{-0.625in} \emph{Using Distributed GPU-Based Parallel-in-Time Integration to \\ Model Particle Production in Analog Cosmologies} \vspace{-0.15in}}
...
\maketitle
    ...
    \begin{center}
    Abstract \\
    Something abstract-like goes here
...
\section{Methods}
    ...
    The methods used in this investigation fall into two different categories: the physical theory of the production of phononic quasiparticles in analog inflationary cosmologies arising from the dynamics of Bose-Einstein condensates, and the computational implementations of high-performance methods such as adaptive integration, finite-differences on non-uniform grids, discretizing differential operators, and the Fast-Fourier Transform.

    \subsection{Physical Theory}

        \subsubsection{BEC Analogs of FLRW Cosmologies}

            The gas as is outlined in section \ref{sec:intro} is a ground-state Bose-Einstein condensate (BEC) without thermal or quantum fluctuations. The dynamics of the BEC are described by the Gross-Pitaevskii equation (GPE) under a Bogoliubov mean-field approximation (also known as the \textit{nonlinear Schr{\"o}dinger equation}),
            
            \begin{equation} \label{eq:GPE}
                i \hbar \, \partial_t \psi(t, \mathbf{x}) = \left[ -\frac{\hbar^2}{2 m} \nabla^2 + V_\text{ext}(x) + U \left| \psi(t, \mathbf{x}) \right|^2 \right] \psi(t, \mathbf{x}).
            \end{equation}
            ...
            \begin{equation} \label{eq:Madelung}
                \psi(t, \mathbf{x}) = \sqrt{n_0 + \Delta n(t, \mathbf{x})} e^{i \left( \theta_0 + \Delta\theta(t, \mathbf{x}) \right)},
            \end{equation}

            where $n_0 + \Delta n(t, \mathbf{x})$ and $\theta_0 + \Delta \theta(t, \mathbf{x})$ are the linearized forms of the real density and phase fields, respectively.  Furthermore, $\Delta n(t, \mathbf{x})$ and $\Delta \theta(t, \mathbf{x})$ are the density and phase perturbations, respectively.
            
            With the Madelung representation defined in equation (\ref{eq:Madelung}), the GPE in equation (\ref{eq:GPE}) becomes\cite{Jain} 
            ...
            Equation (\ref{eq:metric}) can be interpreted as the covariant metric tensor (with determinant $g$) describing an analog, spatially flat, Friedmann–Lema{\^i}tre–Robertson–Walker universe, $c$ is the speed of sound in the condensate, and $d$ is the number of spatial dimensions.  Equation (\ref{eq:KG}) describes both the phase-perturbations $\Delta\theta$ of oscillations with low spatial frequency (i.e. low momentum phonons) in the BEC and also the dynamics of a quantum field that produces massless, spin-0 particles.

        \subsubsection{Variable Speed of Sound and Inflation}

            The time dependence of the system is completely captured in the speed of sound by 

            \begin{equation} \label{eq:speed}
                c(t)^2 = \frac{U(t) n_0}{m} = \frac{4 \pi \hbar^2}{m^2} n_0 \ell(t),
            \end{equation}
        ...
        \subsubsection{The Field Equation}
            ...
            With the scaling function, equation (\ref{eq:KG}) becomes\footnote{$\dot{a}(t) / a(t)$ is the Hubble parameter for an expanding universe with scaling parameter $a(t)$.}

            \begin{subequations}
            \begin{equation} \label{eq:fieldEquation2D}
                \text{2D: } \partial_t^2\Delta\theta - \frac{3}{2} \frac{\dot{a}(t)}{a(t)} \partial_t \Delta\theta - c_0^2 a(t) \nabla^2\Delta\theta = 0
            \end{equation}
            \begin{equation} \label{eq:fieldEquation3D}
                \text{3D: } \partial_t^2\Delta\theta - \phantom{\frac{3}{2}} \frac{\dot{a}(t)}{a(t)} \partial_t \Delta\theta - c_0^2 a(t) \nabla^2\Delta\theta = 0
            \end{equation}
            \end{subequations}

            These equations only differ in the constant coefficient to the first derivative i.e. the dissipative term.  If we consider the \emph{conformal time} $\eta$ defined by $d\eta = \sqrt{a(t)} dt$, the two-dimensional field equation (eq \ref{eq:fieldEquation2D}) becomes

            \begin{equation} \label{eq:fieldEquationConformal}
                \partial_\eta^2 \Delta\theta - \frac{1}{2} \frac{\dot{a}(\eta)}{a(\eta)} \partial_\eta \Delta\theta - c_0^2 \nabla^2 \Delta\theta = 0.
            \end{equation}
...
\vfill
        ...
        \subsubsection{Phononic \& Free-Particle Modes}

            The phase $\Delta\theta(t, \mathbf{x})$ and density $\Delta n(t, \mathbf{x})$ perturbation fields can be expanded in a Fourier plane-wave amplitude representation as 

            \begin{subequations} \label{eq:FourierExpansion}
            \begin{equation}
                \Delta\theta(t, \mathbf{x}) = \frac{1}{\sqrt{V}} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{x}} \widetilde{\theta}_\mathbf{k}(t) \hat{b}_\mathbf{k}(t) + e^{-i \mathbf{k} \cdot \mathbf{x}} \widetilde{\theta}_\mathbf{k}^*(t) \hat{b}_\mathbf{k}^\dagger(t)
            \end{equation}
            \begin{equation}
                \Delta n(t, \mathbf{x}) = \frac{1}{\sqrt{V}} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{x}} \widetilde{n}_\mathbf{k}(t) \hat{b}_\mathbf{k}(t) + e^{-i \mathbf{k} \cdot \mathbf{x}} \widetilde{n}_\mathbf{k}^*(t) \hat{b}_\mathbf{k}^\dagger(t)
            \end{equation}
            \end{subequations}

            where $\mathbf{k}$ is the wave-vector of the phase and density perturbations, $\widetilde{\theta}$ is the discrete Fourier Transform of the phase perturbation, $\hat{b}$ and $\hat{b}^\dagger$ are the annhilation and creation operators, respectively.  From this, there arises two domains to consider\footnote{These domains can also be described as ``acoustic'' and ``Bogoliubov'', respectively.}: phononic quasiparticles with linear dispersion, and free-particle-like quasiparticles with quadratic dispersion.

            The boundary between phononic and free-particle-like quasiparticles is described by the critical wave-number $k_c$.  The critical wave-number $k_c$ is defined\cite{Jain} in terms of the healing length of the BEC $\xi$, where

            \begin{equation} \label{eq:healing}
                \xi(t) = \frac{\hbar}{\sqrt{2} m c} = \xi_0 / \sqrt{a(t)},
            \end{equation}

            and

            \begin{equation} \label{eq:critical}
                k_c(t) = 1 / \xi(t) = \sqrt{a(t)} / \xi_0.
            \end{equation}

            Wave-vectors $\mathbf{k}$ with wave-number $k = ||\mathbf{k}||$ such that $k \ll k_c$ are called \emph{phononic}\footnote{Phononic quasiparticles have linear dispersion $\omega = c k$ characterized by neglecting quantum pressure.}.  Likewise, those with $k \gg k_c$ are called \emph{free-particle-like}\footnote{Free-particle-like quasiparticles have quadratic dispersion characterized by $\omega_k(t)^2 = \frac{k^2}{2m} \left(\frac{\hbar^2 k^2}{2m} + 2 U(t) n_0\right)$ characteried by including quantum-pressure}.

            For phononic quasiparticles, the analog between a BEC and an inflationary cosmology is exact.  Inlcuding free-particle-like quasiparticles forces the analog to include `trans-Planckian' effects and analog Lorentz violation\cite{Jain}.  For these reasons, this investigation focuses only on phononic quasiparticles; namely wave-vectors $\mathbf{k}$ such that $||\mathbf{k}|| \ll k_c$.  Additionally, the free-particle-like regime has nonlinear dispersion which significantly increases the complexity of the field equation\cite{Jain}.

            Only considering phononic modes, the two-dimensional field equation (eq. \ref{eq:fieldEquationConformal}) then becomes

            \begin{equation} \label{eq:fieldEquationConformalFourier}
                \boxed{\partial_\eta^2 \widetilde{\theta}_\mathbf{k} - \frac{1}{2} \frac{\dot{a}(\eta)}{a(\eta)} \partial_\eta \widetilde{\theta}_\mathbf{k} - c_0^2 k^2 \widetilde{\theta}_\mathbf{k} = 0.}
            \end{equation}

            For a de Sitter universe, the scaling function is defined such that $a(t) = \exp(-t / t_s)$.  Therefore, the field equation for a two-dimensional de Sitter universe is

            \begin{equation} \label{eq:fieldEquationdeSitter}
                \partial_\eta^2 \widetilde{\theta}_\mathbf{k} - \frac{1}{\eta} \partial_\eta \widetilde{\theta}_\mathbf{k} + c_0^2 k^2 \widetilde{\theta}_\mathbf{k} = 0,
            \end{equation}
        ...
        \subsubsection{Initial Conditions}
            ...
            \begin{subequations}
                \begin{equation} \label{eq:conditionValue}
                    \widetilde{\theta}_\mathbf{k}(t = 0) = \widetilde{\theta}_{\mathbf{k}0}
                \end{equation}
                \begin{equation} \label{eq:conditionDerivative}
                    \partial_t \widetilde{\theta}_\mathbf{k}(t = 0) = -\frac{U}{\hbar} \widetilde{n}_{\mathbf{k}0}
                \end{equation}
            \end{subequations}

            where the initial derivative of the phase perturbation's Fourier amplitude (eq. \ref{eq:conditionDerivative}) is determined by neglecting the quantum pressure\cite{Jain}.
        ...
        \subsubsection{Particle Production} \label{sec:particleProduction}
            ...
            \begin{equation}
                u_k(t) = \frac{1}{2 \sqrt{n_0}} \widetilde{n}_k(t) + i \sqrt{n_0} \widetilde{\theta}_k(t)
            \end{equation}
            \begin{equation}
                v_k(t) = \frac{1}{2 \sqrt{n_0}} \widetilde{n}_k(t) - i \sqrt{n_0} \widetilde{\theta}_k(t),
            \end{equation}
            \end{subequations}

            ``out'' and ``exp'' corresponding to regions of spacetime outside of and inside the expansion of the universe.

            To summarize, to calculate the number of particles produced at position $\mathbf{x}$ and time $t$ with wave-vector $\mathbf{k}$ looks like the following:

            \begin{enumerate}
                \item Solve the field equation (\ref{eq:fieldEquationConformalFourier}) for $\widetilde{\theta}_\mathbf{k}(t)$
                \item Differentiate $\widetilde{\theta}_\mathbf{k}(t)$ to get $\widetilde{n}_\mathbf{k}(t)$ as in equation (\ref{eq:conditionDerivative})
                \item Combine $\widetilde{\theta}_\mathbf{k}(t)$ and $\widetilde{n}_\mathbf{k}(t)$ as in equations (\ref{eq:mixedFourierAmplitudes})
...
\pagebreak
    ...
    \subsection{Computational Implementation}
        ...
        \subsubsection{The Parareal Algorithm}
            ...
            \begin{itemize}
                \item The Parareal algorithm is used to solve initial value diferential equations in parallel.
                \item It uses a computationally cheap ``coarse'' propagator $\mathcal{C}$ and a more computationally expensive ``fine'' propagator $\mathcal{F}$.
                \item It is an iterative method along the likes of a predictor-corrector, or shooting method.
                \item It converges linearly or superlinearly in iterations.
                \item The basic Parareal algorithm doesn't conserve energy no matter if the coarse and fine propagators are symplectic
                \item Because there are several drawbacks to the basic Parareal algorithm, variants have been developed to address some of these issues.  For example, ParaExp uses exponential matrix integration on linear problems, or so-caled ``Krylov-Enhanced'' methods use geometric methods to make the base algorithm more accurate and stable.
            \end{itemize}

            \begin{algorithm}
                \caption{The Parareal Algorithm}
                \KwIn{Coarse Propagator $\mathcal{C}$, Fine Propagator $\mathcal{F}$, Initial Condition $u_0$, Discretized Time Domain $T = \{T_0, T_1, \ldots, T_N\}$}
                \KwOut{Solution $\{u_0, u_1, \ldots, u_N\}$}
                \tcp*[l]{INITIALIZATION}
                Use the coarse propagator $\mathcal{C}$ and initial value $u_0$ to get data points $\{u_0, u_1, \ldots, u_N\}$ on the discretized time domain $T$\;
                \tcp*[l]{BEGIN PARALLEL}
                \tcp*[l]{parallelize over the temporal subdomains}
                \For{$n = 0, 1, \ldots, N - 1$}{
                    Use the fine propagator $\mathcal{F}$ and psuedo-initial value $u_n$ to get the data point $u_{n+1}^\mathcal{F}$\;
                    Use the coarse propagator $\mathcal{C}$ and psuedo-initial value $u_n$ to get the data point $u_{n+1}^\mathcal{C}$\;
                    $corrector_{n+1} \gets u_{n+1}^\mathcal{F} - u_{n+1}^\mathcal{C}$\;
                }
        ...
        \subsubsection{Field Matrix Equation}
            ...
            solve discretized field equation using multi-GPU-based sparse-matrix equation solver (\verb|cuSOLVER|)

        \subsubsection{Wave Function}

            take the GPU-based GPU-based inverse-fft (\verb|cuFFT|) to get phase and density perturbations to get wave function

        \subsubsection{Density Perturbation}

            differentiate the solution using high-accuracy finite-differences to get density perturbation

        \subsubsection{Particle Production}

            combine to get particle production for a single k

        \subsubsection{All phononic modes}

            repeat/parallelize 4-8 for all k under critical $k_c$
