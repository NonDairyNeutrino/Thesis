...
The two most important concepts presented in @sec:background are: equations of motion, and high-performance computing.  Equations of motion (@sec:background_eom) covers the basics of the mathematical and computional methods that have been used to model physical phenomena, such as the motion of a ball flying through the air.  High-Performance Computing (@sec:background_hpc) covers the fundamentals of using mutliple threads to execute multiple calculations simultaneously, including a high-level description of the GPU/Single-Program-Multiple-Thread model of parallelism, as well as the fundamentals of using multiple processes to achieve further parallelization, including a similarly high-level description of how multiple computers (i.e. unshared memory) can be used.
...
Another point that should be addressed is that the transfer rate between the host and device is not independent of the size of the transfer itself.  On some devices, the transfer rate is nowhere near optimal when then the size of the transfer is below \~2 MB (even with pinned memory), and peak efficiency is only gained with transfers of 16 MB or more @cook2012cuda.  If the device has, say, $N_c = 5 * 10^3$ cores, and they are all being utilized, and in most cases 32-bit floats ($M = 4$ bytes) are used, there is only approximately $N_c M = 4$ MB data to transfer between the host and the device.  While this implementation does not take full advantage of the transfer, it would at least be approaching peak efficiency and more performance will be gained with devices with more compute power.

// - Pinned/Page-locked memory
If the number of transfers is unchanged, the implementation could make use of pinned/page-locked memory.  Pinned memory being memory on the host which the device can access directly without first requesting the host CPU to retrieve it and send it.  Additionally, pinned memory is guarunteed to never /*needs citation*/ be swapped out to disk and thus the device does not need to wait for it to first be transferred to pinned memory @cook2012cuda.  Using this technique could greatly improve performance; the implementation provided in this work does not use it but could be included via library calls @besard2018juliagpu.
